...
* _log-task-execution:                                               Log task execution
...
* create-data-dumps:                                                 Create a full and partial TTL dump of the data generated by the pipeline, as well as the multimedia filename CSV
* create-partial-data-dump:                                          Creates a partial TTL dump of the data generated by the pipeline
...
* download-single-item-for-module:                                   Downloads a single item record for a specific module from MuseumPlus. Pass the module name and the item UUID as command line arguments. Add --force to overwrite existing files.
...
* generate-example-records:                                          Generates example records for e.g. mapping in the X3ML editor
...
* push-latest-data-dumps:                                            Upload full and partial data dumps and CSV files to S3 endpoint
* push-latest-partial-data-dump:                                     Upload latest partial data dump to S3 endpoint for data sharing
* push-multimedia-filename-csv:                                      Upload Multimedia filename CSV to S3 endpoint for data sharing
...
* synchronise-internationalisation:                                  Synchronises the internationalisation files with the messages.properties file. Pass the DEEPL API key as a CLI argument or through the environment variable DEEPLKEY.
